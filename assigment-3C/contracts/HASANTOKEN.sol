...
//5000000000000000000000  
    ...
    ERC20Capped(cap)
    {
     ...
     function getContractBalance() public view returns(uint) {
        ...
        // }    
          ...
          function balanceeeOf(address account) public view returns (uint256) {
    ...
    function buyTokens() public payable returns(bool){
       require (msg.sender != address(0) && msg.sender!=owner(), "you are not a valid token holder");
       require(msg.value >= 0.01 ether, "Error, deposit must be >=0.01");
       uint256   amount  =msg.value / tokenPrice;   // 1 ether = 1000 HT token
       require( totalSupply()>= amount*10**18 ,'smart contract dont hold enough tokens');
  ...
  function returnToken(uint _tokenCountReturned) public  payable returns (bool) {
      ...
      require(_balances[msg.sender] >=_tokenCountReturned,"you have not enough tokens");
      
      uint weiReturned  = tokenPrice * _tokenCountReturned ;
       require( getContractBalance()>=weiReturned,'contract has not  balance');

       _balances[msg.sender]-=_tokenCountReturned*10**18;
       _balances[admin]+=_tokenCountReturned*10**18;
