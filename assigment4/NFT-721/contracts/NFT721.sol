...
contract NFT721 is Ownable, ERC721 {
   ...
   /// 3. the owner can set base URI 
    ...
    function mintNFT(string memory tokenURI) public onlyOwner returns(uint8, string memory) {
    ...
    modifier isSaleOn() {
        require( block.timestamp >= startSaleTime && block.timestamp <=  endSaleTime, "Sorry, not a Sale period");
        _;
    }
